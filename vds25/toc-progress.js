...
{
    var selected_code = ' {font-weight: bold; text-color: #ffffff}\n'
	// Create the skeleton
    ...
    var main_sections = document.querySelectorAll(".slides > section:not(.no-toc-progress)")
	...
	for (var main_sections_index=0;main_sections_index<main_sections.length;main_sections_index++)
	{
		...
		var main_title_set = false;
		{
			...
			if (title_element!=null && (!title_element.hasAttribute('class') || title_element.getAttribute('class').indexOf('no-toc-progress')==-1))
			{
                ...
                li_element.classList.add('toc-main-item')
                li_elems.push(li_element)
				...
				style_node.textContent=style_node.textContent+'.toc-progress-'+main_sections_index.toString()+' #toc-progress-'+main_sections_index.toString()+selected_code;
				style_node.textContent=style_node.textContent+this.viewport+'[class*="toc-progress-'+main_sections_index.toString()+'-"] #toc-progress-'+main_sections_index.toString()+selected_code;
...
toc_progress.reduceorscrollifnecessary=function()
{
	if (this.toc_progress_on==true)
	{
        ...
        var mains = document.getElementById('toc-progress-footer-main').getElementsByTagName('li')
        for(const m of mains) {
			if(body_classes.contains(m.id)) {
				m.classList.add('toc-active')
                m.classList.remove('toc-inactive')
            } else {
                m.classList.remove('toc-active')
                m.classList.add('toc-inactive')
            }
        }
